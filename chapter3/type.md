...
當在release模式時，Rust不會檢查，而會做「二補數包裝」(two's complement wrapping)。簡言之，`256`會變`0`，`257`會變`1`等等。通常溢位會被當成錯誤，就算是「二補數包裝」後也一樣。如果確定要使用這方法，標準函式庫有一個型態`Wrapping`可以標示這行為而不會被認定為錯誤。
...
```
這段程式先將數值組綁在變數`tup`上，再用組合了類型的`let`將`tup`分別給了三個變數`x`、`y`和`z`。這稱為**解構**，因為它將一個數值組分解成三個部分。最後，程式把`y`的值印出。
...
let a = [1, 2, 3, 4, 5];
```
當想把資料放在stack而不是heap中，或是想確定數量一定會固定時，陣列很好用。陣列與第八章會提到的vector很像，但是vector可以伸縮長度。當你不知道要用陣列還是vector時，vector是你的好朋友。
